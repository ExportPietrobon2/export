...
async function carregar() {
 ...
 return `
 <div class="card border-0 shadow-sm mb-2"><div class="card-body"><div class="d-flex justify-content-between align-items-start flex-wrap gap-2 mb-1"><div><div class="fw-bold">${d.descricao}</div><div class="small text-muted">
 ${d.quantidade > 0 ? `${d.quantidade} ${d.unidade || ''}` : ''}
 ${d.numero_pi ? ` · PI ${d.numero_pi}` : ''}
 ${d.solicitante ? ` · Solicitante: ${d.solicitante}` : ''}
 </div></div><div class="d-flex gap-1 flex-wrap justify-content-end"><span class="badge ${cat.cor}">${cat.label}</span><span class="badge ${st.cor}">${st.label}</span></div></div>
 ${d.respondido_por ? `<div class="small text-muted mb-2">Respondido por ${d.respondido_por}</div>` : ''}
 ${podeResponder ? `<div class="d-flex gap-2 flex-wrap"><button class="btn btn-sm ${d.status === 'tem' ? 'btn-success' : 'btn-outline-success'}" onclick="responderPedidoCompra(${d.id}, 'tem')">Tenho</button><button class="btn btn-sm ${d.status === 'nao_tem' ? 'btn-danger' : 'btn-outline-danger'}" onclick="responderPedidoCompra(${d.id}, 'nao_tem')">Não tenho</button></div>` : ''}
